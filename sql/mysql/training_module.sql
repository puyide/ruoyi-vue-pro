...
CREATE TABLE `training_template` (
    ...
    `code` VARCHAR(20) NOT NULL COMMENT '模板编码，如 S01, L03',
    `domain` VARCHAR(50) NOT NULL COMMENT '训练域: social/language/emotion/cognition/sensory/daily_living',
    `name` VARCHAR(100) NOT NULL COMMENT '训练名称',
    `brief_goal` TEXT COMMENT '简要目标（一句话）',
    `recommended_frequency` VARCHAR(100) COMMENT '推荐训练频率',
    `base_scenario` TEXT COMMENT '基础场景描述',
    `level1_rules` TEXT COMMENT 'Level 1 规则（JSON）',
    `level2_rules` TEXT COMMENT 'Level 2 规则（JSON）',
    `level3_rules` TEXT COMMENT 'Level 3 规则（JSON）',
    `extra_meta` TEXT COMMENT '额外元数据（JSON）',
    ...
    PRIMARY KEY (`id`),
    INDEX `idx_training_template_domain` (`domain`),
    INDEX `idx_training_template_code` (`code`),
    UNIQUE INDEX `uk_training_template_code` (`code`)
...
CREATE TABLE `training_session` (
    ...
    `scheduled_at` DATETIME COMMENT '计划训练时间',
    `difficulty_level` INT DEFAULT 1 COMMENT '难度等级 1/2/3',
    `ai_input_json` TEXT COMMENT '传给大模型的输入JSON',
    `ai_output_json` TEXT COMMENT '大模型生成的训练卡片JSON',
    `status` TINYINT DEFAULT 0 COMMENT '状态: 0-已计划 1-进行中 2-已完成 3-已跳过',
    `started_at` DATETIME COMMENT '开始时间',
    `completed_at` DATETIME COMMENT '完成时间',
    `actual_duration_minutes` INT COMMENT '实际训练时长（分钟）',
    `creator` VARCHAR(64) DEFAULT '' COMMENT '创建者',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updater` VARCHAR(64) DEFAULT '' COMMENT '更新者',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    ...
    `tenant_id` BIGINT DEFAULT 0 COMMENT '租户编号',
    PRIMARY KEY (`id`),
    INDEX `idx_training_session_child_id` (`child_id`),
    INDEX `idx_training_session_user_id` (`user_id`),
    INDEX `idx_training_session_template_id` (`template_id`),
    INDEX `idx_training_session_scheduled_at` (`scheduled_at`),
...
CREATE TABLE `training_log` (
    ...
    `child_id` BIGINT NOT NULL COMMENT '孩子ID',
    `user_id` BIGINT NOT NULL COMMENT '用户ID（家长）',
    `template_id` BIGINT COMMENT '模板ID（冗余）',
    `domain` VARCHAR(50) COMMENT '训练域（冗余）',
    `done_at` DATETIME NOT NULL COMMENT '打卡时间',
    `completed` TINYINT(1) DEFAULT 1 COMMENT '是否完成',
    `child_mood` TINYINT COMMENT '孩子情绪: 1-还不错 2-一般 3-情绪不好',
    `success_count` INT COMMENT '成功次数',
    `difficulties` TEXT COMMENT '遇到的困难（JSON数组）',
    `parent_comment` TEXT COMMENT '家长备注',
    `structured_data` TEXT COMMENT '结构化数据（JSON）',
    `shared_to_community` TINYINT(1) DEFAULT 0 COMMENT '是否分享到社区',
    `ai_feedback` TEXT COMMENT 'AI反馈内容（JSON）',
    `creator` VARCHAR(64) DEFAULT '' COMMENT '创建者',
    `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `updater` VARCHAR(64) DEFAULT '' COMMENT '更新者',
    `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    ...
    `tenant_id` BIGINT DEFAULT 0 COMMENT '租户编号',
    PRIMARY KEY (`id`),
    INDEX `idx_training_log_session_id` (`session_id`),
    INDEX `idx_training_log_child_id` (`child_id`),
    INDEX `idx_training_log_user_id` (`user_id`),
    INDEX `idx_training_log_template_id` (`template_id`),
    INDEX `idx_training_log_done_at` (`done_at`),
    INDEX `idx_training_log_domain` (`domain`)
...
-- 社交与互动（S01-S10）
INSERT INTO `training_template` (`id`, `code`, `domain`, `name`, `brief_goal`, `recommended_frequency`, `base_scenario`, `level1_rules`, `level2_rules`, `level3_rules`, `sort`) VALUES
(1, 'S01', 'social', '回应名字', '当家长叫名字时，孩子能有转头、停顿或看向家长的反应。', '每天 5-10 次，穿插在日常生活中', '在家中日常活动场景中，通过多次、有强化的叫名练习，建立"名字=注意到大人"的联系。', 
...
(2, 'S02', 'social', '共同注意：跟随指物', '当大人指向某个物品时，孩子能把视线转向该物品。', '每天 3-5 次', '在有吸引力物品的环境中，引导孩子跟随大人的手指方向看向目标。',
...
(3, 'S03', 'social', '展示物品给大人看', '孩子愿意把手里的东西拿给大人看，建立分享的习惯。', '每天 3 次以上', '当孩子拿到感兴趣的物品时，引导他展示给大人看。',
'{"prompt": "大人主动靠近表示兴趣", "success_criteria": "允许大人看物品"}',
'{"prompt": "语言引导给我看看", "success_criteria": "主动举起展示"}',
'{"prompt": "无提示", "success_criteria": "自发展示新发现"}', 3),

(4, 'S04', 'social', '轮流：你一下我一下', '孩子能和大人进行简单的轮流游戏（比如轮流丢球）。', '每天至少 1 个短局', '用孩子喜欢的玩具，建立轮流互动的模式。',
...
(5, 'S05', 'social', '模仿：拍手', '孩子能模仿大人的拍手动作。', '每天 5 次左右', '通过示范和强化，建立动作模仿的意识。',
...
(6, 'S06', 'social', '模仿：敲桌子/地面', '孩子能模仿大人的敲击动作，提升模仿和共同活动。', '每天 3-5 次', '用有节奏的敲击游戏，引导孩子跟随模仿。',
...
(7, 'S07', 'social', '短暂对视 1 秒', '孩子能短暂地看向大人眼睛或脸部。', '每天在自然互动中多次尝试', '在孩子需要帮助或期待奖励时，引导建立短暂对视。',
...
(8, 'S08', 'social', '主动拍大人引起注意', '孩子想要什么时，可以主动拍大人而不是只是哭或拉扯。', '每天观察和捕捉自然机会', '当孩子有需求时，引导用拍打方式引起注意。',
...
(9, 'S09', 'social', '拉大人去需要的地方', '孩子有需要时，能主动拉大人的手往目标方向走。', '每天 2-3 次', '创造需要帮助的场景，引导孩子用拉手表达需求。',
...
(10, 'S10', 'social', '挥手打招呼/再见', '在见面或离开时，孩子能模仿大人挥手。', '每天在真实场景中练习', '利用日常出入场景，建立挥手的社交习惯。',
'{"prompt": "手把手带挥", "timing": "分离时", "success_criteria": "配合挥手"}',
'{"prompt": "示范", "timing": "见面/离开", "success_criteria": "模仿挥手"}',
'{"prompt": "语言", "timing": "各种场合", "success_criteria": "主动挥手"}', 10);

-- 语言与沟通（L01-L10）
INSERT INTO `training_template` (`id`, `code`, `domain`, `name`, `brief_goal`, `recommended_frequency`, `base_scenario`, `level1_rules`, `level2_rules`, `level3_rules`, `sort`) VALUES
(11, 'L01', 'language', '听懂"一起"', '孩子能理解"我们一起做某件事"的口令并参与行动。', '每天 3-5 次', '在游戏或活动开始时，用"一起"引导共同参与。',
...
(12, 'L02', 'language', '用伸手表达想要', '孩子想要东西时，用伸手来表达，而不是只哭闹。', '每天多次', '在孩子想要物品时，等待并引导伸手表达。',
...
(13, 'L03', 'language', '用指物表达想要', '孩子能用手指指向目标物品，表达自己的需求。', '每天 3-5 次', '将物品放在孩子触及不到的地方，引导指向表达。',
...
(14, 'L04', 'language', 'PECS：递图片换物品', '孩子通过递交图片给大人来获得想要的物品。', '每天 3-5 次', '使用图片交换系统，建立沟通桥梁。',
...
(15, 'L05', 'language', '理解指令"给我"', '孩子能理解"给我"的指令并把物品递给大人。', '每天 5 次左右', '在自然互动中练习物品传递。',
...
(16, 'L06', 'language', '理解"不可以"', '孩子能逐渐理解"不可以"的含义，学会在被制止时停下来。', '根据实际情况出现时使用', '在危险或不当行为时，用清晰的语言和动作制止。',
...
(17, 'L07', 'language', '模仿拟声词', '孩子能模仿简单的动物叫声或拟声词（如"哞""咩"）。', '每天 3-5 次游戏式练习', '通过动物图片或玩具，引导模仿发声。',
...
(18, 'L08', 'language', '声音轮流游戏', '孩子能和大人轮流发声，形成初步"你一句我一句"的互动。', '每天 1-2 轮', '用简单的声音游戏建立语言轮流意识。',
...
(19, 'L09', 'language', '说"要"来请求', '孩子能用"要"这个词来表达需求（或相应发音/替代词）。', '每天多次', '在孩子有需求时，等待并引导说出"要"。',
...
(20, 'L10', 'language', '双词组合：要+物品', '孩子能用两词短语表达需求，例如"要 饼干"。', '每天 3-5 次', '在孩子说出单词后，引导扩展为两词组合。',
...
-- 情绪与行为（E01-E10）
INSERT INTO `training_template` (`id`, `code`, `domain`, `name`, `brief_goal`, `recommended_frequency`, `base_scenario`, `level1_rules`, `level2_rules`, `level3_rules`, `sort`) VALUES
(21, 'E01', 'emotion', '等待 5 秒', '孩子在得到想要的物品前，能短暂等待约 5 秒。', '每天 3-5 次', '在给予物品前，引导短暂等待。',
...
(22, 'E02', 'emotion', '情绪识别：高兴', '孩子能在图片或真人表情中分辨"高兴"的情绪。', '每周多次', '用表情图片或镜子，帮助识别高兴的表情。',
'{"material": "夸张图", "task": "指认", "success_criteria": "能指出"}',
'{"material": "真人照", "task": "选择", "success_criteria": "正确选择"}',
'{"material": "视频/真人", "task": "判断", "success_criteria": "说出高兴"}', 22),

(23, 'E03', 'emotion', '情绪识别：生气', '孩子能在图片或场景中分辨"生气"的表情或情绪。', '每周多次', '用表情图片帮助识别生气的情绪。',
'{"material": "夸张图", "task": "指认", "success_criteria": "能指出"}',
'{"material": "真人照", "task": "选择", "success_criteria": "正确选择"}',
'{"material": "情境", "task": "判断", "success_criteria": "理解情境"}', 23),

(24, 'E04', 'emotion', '用拍大人替代尖叫', '孩子在有需求时，学会用拍大人来表达，而不是用尖叫。', '每天捕捉机会强化', '在孩子尖叫时，立即引导替代行为。',
...
(25, 'E05', 'emotion', '深呼吸 3 次', '孩子在紧张或不舒服时，能在大人带领下做 3 次深呼吸。', '每天 1 次练习，情绪激动时使用', '平时练习深呼吸，情绪激动时使用。',
...
(26, 'E06', 'emotion', '先做 A 再做 B', '孩子能理解"先……再……"的简单任务顺序。', '每天 1-2 次', '用视觉提示帮助理解任务顺序。',
'{"task": "简单", "visual": "图卡", "success_criteria": "按顺序做"}',
'{"task": "日常", "visual": "语言", "success_criteria": "理解顺序"}',
'{"task": "变化", "visual": "无", "success_criteria": "灵活执行"}', 26),

(27, 'E07', 'emotion', '用摇头表达"不"', '孩子能用摇头来表达自己不想要某物。', '每天观察机会练习', '在提供不喜欢的东西时，引导用摇头表达。',
...
(28, 'E08', 'emotion', '转移注意到新活动', '当孩子过度沉迷某一活动时，大人能带领孩子转移到新的简单活动。', '根据情况使用', '使用预告和吸引策略帮助转移。',
...
(29, 'E09', 'emotion', '把大任务拆成小任务', '面对困难任务时，将其拆成多个小步骤。', '遇到大任务时使用', '用视觉清单帮助理解任务分解。',
...
(30, 'E10', 'emotion', '安静坐 10 秒', '孩子能在桌边或椅子上安静坐约 10 秒。', '每天 1-2 次', '在结构化活动中练习安坐。',
...
-- 认知与规则（C01-C10）
INSERT INTO `training_template` (`id`, `code`, `domain`, `name`, `brief_goal`, `recommended_frequency`, `base_scenario`, `level1_rules`, `level2_rules`, `level3_rules`, `sort`) VALUES
(31, 'C01', 'cognition', '颜色匹配', '孩子能将相同颜色的卡片或物品配对在一起。', '每周 3-5 次', '使用颜色鲜明的材料进行匹配练习。',
...
(32, 'C02', 'cognition', '分类：吃的 vs 玩的', '孩子能区分哪些是可以吃的东西，哪些是玩具。', '每周 3 次', '用实物或图片进行分类练习。',
...
(36, 'C06', 'cognition', '听从 1 步指令', '孩子能理解并完成简单的一步指令。', '每天多次自然练习', '在日常中给予简单指令并强化完成。',
...
(37, 'C07', 'cognition', '听从 2 步指令', '孩子能按照顺序完成两步指令。', '每天 1-2 次', '给予连续两步的简单指令。',
...
(38, 'C08', 'cognition', '图形匹配：圆和方', '孩子能区分并匹配圆形和方形。', '每周 3 次', '用图形板或卡片练习匹配。',
...
(39, 'C09', 'cognition', '规则游戏：停和走', '孩子能理解"走"和"停"的游戏规则。', '每周多次', '用音乐或口令进行停走游戏。',
...
(40, 'C10', 'cognition', '记忆：找一找藏起来的物品', '孩子能记住刚刚藏起的物品位置并找出来。', '每周 3 次', '用藏找游戏训练工作记忆。',
...
-- 感觉与运动（G01-G10）
INSERT INTO `training_template` (`id`, `code`, `domain`, `name`, `brief_goal`, `recommended_frequency`, `base_scenario`, `level1_rules`, `level2_rules`, `level3_rules`, `sort`) VALUES
(41, 'G01', 'sensory', '平衡木走 3 步', '孩子能在简单"平衡线"或窄面上走几步。', '每周 3-5 次', '用地上的线或低矮平衡木练习。',
...
(42, 'G02', 'sensory', '推墙 10 秒', '孩子用力推墙几秒钟，提供本体觉输入。', '每天 1-2 次', '在需要调节时使用推墙活动。',
...
(43, 'G03', 'sensory', '触觉探索：软硬不同的物体', '孩子通过摸不同材质的物品，逐渐适应不同触觉。', '每周 3 次', '提供各种触感材料让孩子探索。',
...
(44, 'G04', 'sensory', '双脚原地跳', '孩子能尝试双脚一起离地的小跳。', '每周 3-5 次', '用游戏方式引导原地跳跃。',
...
(45, 'G05', 'sensory', '跨过小障碍物', '孩子能迈步跨过低矮障碍物。', '每周 3 次', '设置低矮障碍让孩子练习跨越。',
...
(46, 'G06', 'sensory', '精细：把吸管插进洞里', '孩子能把细长的物体准确插入小孔中。', '每周 3-5 次', '用插孔玩具练习手眼协调。',
...
(47, 'G07', 'sensory', '精细：捏夹子夹起物品', '孩子用手指捏夹子夹起小物品。', '每周 3 次', '用晾衣夹等工具练习捏夹。',
...
(48, 'G08', 'sensory', '手眼协调：丢球进桶', '孩子能尝试把球丢进桶里。', '每周 3-5 次', '用大桶和球练习投掷。',
...
(49, 'G09', 'sensory', '身体认知：摸头摸鼻子', '孩子能在大人指令下摸自己的身体部位。', '每天多次自然带练', '在歌曲或游戏中练习身体认知。',
'{"parts": "头", "prompt": "示范", "success_criteria": "模仿摸"}',
'{"parts": "3个", "prompt": "语言", "success_criteria": "听令摸"}',
'{"parts": "多个", "prompt": "快速", "success_criteria": "准确快速"}', 49),

(50, 'G10', 'sensory', '节奏模仿：敲敲停', '孩子能模仿简单的敲击节奏。', '每周多次短游戏', '用敲击游戏训练节奏感。',
...
-- 生活自理（D01-D10）
INSERT INTO `training_template` (`id`, `code`, `domain`, `name`, `brief_goal`, `recommended_frequency`, `base_scenario`, `level1_rules`, `level2_rules`, `level3_rules`, `sort`) VALUES
(51, 'D01', 'daily_living', '洗手三步：开水→搓→冲', '孩子在大人提示下，能按顺序完成洗手三个核心步骤。', '每天至少 1 次', '每次洗手时按步骤引导。',
...
(52, 'D02', 'daily_living', '穿鞋：区分左右', '孩子逐渐理解鞋子的左右。', '每天外出前练习', '用标记帮助识别左右鞋。',
...
(57, 'D07', 'daily_living', '收拾玩具：放回盒子', '在大人带领下，孩子能把散落的玩具收回指定容器。', '每天玩耍结束时', '玩耍后引导收拾。',
...
(58, 'D08', 'daily_living', '刷牙：张嘴 5 秒', '孩子能在大人操作牙刷时，配合张嘴保持几秒。', '每天 1-2 次', '刷牙时练习配合。',
...
(59, 'D09', 'daily_living', '开关灯', '孩子能在指令下自己去开灯或关灯。', '每天自然情境中使用', '利用日常情境练习。',
...
(60, 'D10', 'daily_living', '拉拉链向上 5cm', '孩子能尝试抓住拉链头并向上拉一小段距离。', '每天穿脱外套时练习', '穿脱衣服时练习拉链。',
